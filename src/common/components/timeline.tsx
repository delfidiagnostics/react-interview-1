        ...
        <nav aria-label="Progress">
            <ol role="list" className="overflow-hidden">
                {timeline.map((step, stepIdx) => (
                    <li key={step.name}
                        className={classNames(stepIdx !== timeline.length - 1 ? 'pb-10' : '', 'relative')}>
                        {step.status === 'complete' ? (
                            <>
                                {stepIdx !== timeline.length - 1 ? (
                                    <div className="-ml-px absolute mt-0.5 top-4 left-4 w-0.5 h-full bg-indigo-600"
                                         aria-hidden="true"/>
                                ) : null}
                                <a href={step.href} className="relative flex items-start group">
                                  <span className="h-9 flex items-center">
                                    <span
                                        className="relative  w-8 h-8 flex items-center justify-center bg-indigo-600 rounded-full group-hover:bg-indigo-800">
                            ...
                            <>
                                ...
                                <a href={step.href} className="relative flex items-start group" aria-current="step">
                                  <span className="h-9 flex items-center" aria-hidden="true">
                                    ...
                                    <span className="ml-4 min-w-0 flex flex-col">
                                    <span
                                        className="text-xs font-semibold tracking-wide uppercase text-indigo-600">{step.name}</span>
                                    <span className="text-sm text-gray-500">{step.content}</span>
                                  </span>
                                </a>
                            </>
